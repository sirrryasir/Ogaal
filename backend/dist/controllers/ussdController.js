...
const sendSMS = async (to, message) => {
    try {
        ...
        const authKey = crypto
            ...
            .digest('hex');
        const response = await axios.post('https://sms.mytelesom.com/index.php/smsapi/v1/messages', {
            to: [to],
            message,
            type: 'unicode',
...
const getPaginatedUssdMenu = async (model, page = 1, where = {}, title, backText = "0. Dib ugu noqo", includeBack = true) => {
    const pageSize = 10;
...
const handleUssdRequest = async (req, res) => {
    ...
    try {
        ...
        if (step === 0) {
            const result = await getPaginatedUssdMenu(prisma.region, 1, {}, "Kusoo dhawoow Ogaal\nDooro Gobol:", "0. Exit", false);
        ...
        // Handle pagination navigation (Next/Previous)
            ...
            if (step === 1) {
                ...
                if (parts[0].includes('page')) {
                    ...
                    if (pageMatch) {
                        ...
                        const result = await getPaginatedUssdMenu(prisma.region, page, {}, `Kusoo dhawoow Ogaal\nDooro Gobol (Page ${page}):`, "0. Dib ugu noqo");
                        response = result.response;
                    }
                }
                // Regular region selection (1-10, 11-20, etc.)
                    ...
                    if (inputNum <= allRegions.length) {
                        ...
                        const result = await getPaginatedUssdMenu(prisma.district, 1, { region_id: selectedRegion.id }, `Gobolka: ${selectedRegion.name}\nDooro Degmo:`, "0. Dib ugu noqo gobollada");
                        response = result.response;
                    }
                    else {
            ...
            // STEP 2: Handle district selection or pagination
                ...
                if (districtInput.includes('page')) {
                    ...
                    if (pageMatch) {
                        ...
                        const result = await getPaginatedUssdMenu(prisma.district, page, { region_id: selectedRegion.id }, `Gobolka: ${selectedRegion.name}\nDooro Degmo (Page ${page}):`, "0. Dib ugu noqo");
                        response = result.response;
                    }
                }
                else {
                    ...
                    if (districtNum > 0 && districtNum <= allDistricts.length) {
                        ...
                        const result = await getPaginatedUssdMenu(prisma.village, 1, { district_id: selectedDistrict.id }, `Degmada: ${selectedDistrict.name}\nDooro Tuulo:`, "0. Dib ugu noqo degmooyinka");
                        response = result.response;
                    }
            ...
            // STEP 3: Handle village selection or pagination
                ...
                if (villageInput.includes('page')) {
                    ...
                    if (pageMatch) {
                        ...
                        const result = await getPaginatedUssdMenu(prisma.village, page, { district_id: selectedDistrict.id }, `Degmada: ${selectedDistrict.name}\nDooro Tuulo (Page ${page}):`, "0. Dib ugu noqo");
                        response = result.response;
                    }
                }
                else {
            ...
            // STEP 4: Handle action selection
                ...
                if (action === "1") {
                    ...
                    const result = await getPaginatedUssdMenu(prisma.waterSource, 1, { village_id: selectedVillage.id }, `Tuulada: ${selectedVillage.name}\nDooro Isha Biyaha:`, "0. Dib ugu noqo");
            ...
            // STEP 5: Handle final selections
                ...
                if (action === "1") {
                    if (selection.includes('page')) {
                        ...
                        if (pageMatch) {
                            const page = parseInt(pageMatch[1]);
                            const result = await getPaginatedUssdMenu(prisma.waterSource, page, { village_id: selectedVillage.id }, `Tuulada: ${selectedVillage.name}\nDooro Isha Biyaha (Page ${page}):`, "0. Dib ugu noqo");
                            response = result.response;
                        }
                    }
                    else {
