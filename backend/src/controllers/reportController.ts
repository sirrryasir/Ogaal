...
const create = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!village_id || !water_source_id || !isContentValid) {
      console.log("Validation failed:", {
        ...
        contentTrimmed: typeof content === 'string' ? content.trim() : 'N/A',
      });
      res.status(400).json({
        message:
          "Missing required fields: village_id, water_source_id, and content (must be a non-empty string) are required",
      });
      return;
    }
