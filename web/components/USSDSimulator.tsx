...
const USSDSimulator = () => {
  ...
  const sendRequest = async (sid: string, text: string) => {
    try {
      ...
      const res = await fetch("https://barwaaqo.onrender.com/ussd", {
        method: "POST",
    ...
    <div className="flex flex-col items-center justify-center p-4 bg-gray-100 rounded-3xl w-[320px] h-[640px] border-4 border-gray-800 shadow-2xl relative">
      ...
      <div className="w-full h-[60%] bg-blue-500 rounded-t-xl overflow-hidden relative mb-4 flex flex-col items-center justify-center text-white p-4">
        ...
        {sessionActive ? (
          <div className="bg-white text-black w-full h-[80%] p-2 rounded shadow flex flex-col">
            <pre className="whitespace-pre-wrap font-mono text-sm flex-grow">
              {screenText}
            </pre>
            ...
            <div
              className="text-right text-xs text-blue-600 mt-1 cursor-pointer"
      ...
      <div className="grid grid-cols-3 gap-4 w-full px-4">
        <button
          ...
          className="col-span-1 bg-red-500 text-white rounded p-2 text-sm font-bold"
        >
          End
        </button>
        <button
          onClick={() => handleKey("Call")}
          className="col-span-1 bg-green-500 text-white rounded p-2 text-sm font-bold"
        >
          Call/Send
        </button>
        <button
          onClick={() => handleKey("Del")}
          className="col-span-1 bg-gray-400 text-white rounded p-2 text-sm font-bold"
        >
          Del
        </button>
      </div>
